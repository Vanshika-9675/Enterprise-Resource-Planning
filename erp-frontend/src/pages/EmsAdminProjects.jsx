...
const EmsAdminProjects = () => {
  ...
  const [projectPerPage] = useState(5);
  ...
  const currentProjects = project.slice(indexOfFirstProject, indexOfLastProject);
  ...
  const handleUpdateProject = (status, e) => {
    e.preventDefault();
    const updatedProjectData = {
      status,
    };
    dispatch(
      updateProject({ projectId: CurrentId, projectData: updatedProjectData })
    ).unwrap()
      .then(() => {
        dispatch(fetchProjects());
      })
                    ...
                    <tbody>
                      {currentProjects.map((project) => (
                        <tr key={project.projectId}>
                          ...
                          <td className="border px-4 py-2 text-center text-xl text-[#f9c906]">
                            <button
                              onClick={() =>
